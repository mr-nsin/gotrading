...
class KotakNeoDataPipeline(BaseDataPipeline):
    """
    Connects to Kotak Neo Live WebSocket and pushes parsed ticks into an asyncio.Queue.
    """
    def __init__(self, symbols: List[str], event_queue: asyncio.Queue, loop: asyncio.AbstractEventLoop):
        ...
        if not self.client_code:
            logger.warning("Kotak Neo credentials missing. Cannot connect to Live WebSocket.")
    ...
    def _on_message(self, ws, message):
        ...
        ltp = message.get("ltp") or message.get("lastPrice")
        if ltp:
            event = {
                "type": "TICK",
                "data": message,
    ...
    def connect(self):
        ...
        while self._running:
            try:
                logger.info(f"Connecting to Kotak Neo Live Data Pipeline for {self.symbols}...")
                time.sleep(5)
            except Exception as e:
                logger.error(f"Kotak Neo Stream Error: {e}. Reconnecting in 5s...")
                time.sleep(5)
